...
% 	\begin{figure}[h]
% 		\centering
% 		\includegraphics[width=0.8\linewidth]{images/SVM/f1_score.png}
% 	\end{figure}
% \end{frame}
...
\begin{itemize}[<+->]
	\item From the input preprocessors the most effective was TF-IDF
	\item The best model was neural network, with random forest and logistic regression not far behind \\
	(additionally neural network was the most time consuming to train)
	\item KNN indeed was far from the best model \\
	but naive bayes, which was the fastest, was quite good
	\item Decision trees were empirically proven (again) that they are not the best choice
\end{itemize}
\end{frame}
